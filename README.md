...
└── src/
    ...
    ├── firebase.js             # Legacy Firebase content backend (unused — placeholder keys)
    ├── context/
    ...
    └── assets/                 # Legacy Vite template assets
```
...
| `/admin` | Admin | Password-protected content editor (password: `[REDACTED]`) |
| `*` | Home | Fallback |
...
> Note: `src/firebase.js` is a legacy content backend with placeholder credentials and is **not** imported anywhere. It can be removed.
...
- The **admin password** is hardcoded in `src/pages/Admin.jsx:12` — move it to a secure server-side check or environment variable before production.
...
- Several sections/pages are legacy or unused (e.g. `components/Hero.jsx`, `CalendlyWidget.jsx`, `sections/Insurance.jsx`, `sections/Providers.jsx`, `sections/Testimonials.jsx`, `pages/ServiceAcuteCare.jsx`, `ServiceChronicDisease.jsx`, `ServicePreventiveCare.jsx`, `App.css`) and can be cleaned up.
